...
describe("Services API route", () => {
  ...
  it("GET rejects invalid query params", async () => {
    const response = await GET(
      createRequest("http://localhost/api/services?take=-5"),
    );
  ...
  it("POST surfaces handler errors", async () => {
    vi.mocked(createPerformedService).mockRejectedValueOnce(
      new Error("db down"),
    );
  ...
  it("PUT updates a service when payload is valid", async () => {
    ...
    expect(jsonSpy).toHaveBeenCalledTimes(1);
    expect(updatePerformedService).toHaveBeenCalledWith(body.id, {
      amount: 99,
    });
    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toEqual({
  ...
  it("PUT rejects invalid payloads", async () => {
    const [request] = createJsonRequest("http://localhost/api/services", {
      amount: 5,
    });
  ...
  it("DELETE removes a service when id is valid", async () => {
    ...
    const response = await DELETE(
      createRequest(
        "http://localhost/api/services?id=3fa85f64-5717-4562-b3fc-2c963f66afa6",
      ),
    );

    expect(deletePerformedService).toHaveBeenCalledWith(
  ...
  it("DELETE surfaces handler errors", async () => {
    ...
    const response = await DELETE(
      createRequest(
        "http://localhost/api/services?id=16fd2706-8baf-433b-82eb-8c7fada847da",
      ),
    );
